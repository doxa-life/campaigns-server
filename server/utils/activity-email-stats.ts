...
export async function collectActivityStats(periodStart: Date, periodEnd: Date): Promise<ActivityStats> {
  ...
  const [
    ...
    db.prepare(`SELECT COALESCE(SUM(prayer_duration), 0) as daily_total FROM campaign_subscriptions WHERE status = 'active'`).get(),
  ...
  return {
    ...
    prayerCommitted: Number(prayerCommittedRow?.daily_total ?? 0) * Math.round((periodEnd.getTime() - periodStart.getTime()) / (1000 * 60 * 60 * 24)),
